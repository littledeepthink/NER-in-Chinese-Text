...
def evaluation(pred_entity, true_entity):
    ...
    precision = sum(t_pos) / sum(pred)
    recall = sum(t_pos) / sum(true)
...
if __name__ == '__main__':
    ...
    # ## loss:16.72; 0.5966 0.5158 0.5532
    # ner_model = BiLSTM_CRF(n_input=200, n_vocab=embedding_mat.shape[0], n_embed=100,
    #                        embedding_mat=embedding_mat, keep_prob=0.5,
    #                        n_lstm=128, keep_prob_lstm=0.95, n_entity=7,
    #                        optimizer='adam', batch_size=64, epochs=500)

    # ## loss: 16.68706; precision:0.7474, recall:0.7302, F1:0.7387
    # ner_model = BiLSTM_CRF(n_input=200, n_vocab=embedding_mat.shape[0], n_embed=100,
    #                        embedding_mat=embedding_mat, keep_prob=0.5,
    #                        n_lstm=100, keep_prob_lstm=0.8, n_entity=7,
    #                        optimizer='adam', batch_size=64, epochs=500)
    
    ## loss: 16.68228; precision:0.7741 recall:0.7641 F1:0.7691
